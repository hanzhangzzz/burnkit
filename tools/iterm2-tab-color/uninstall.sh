...
用法:
  bash tools/iterm2-tab-color/uninstall.sh [选项]

选项:
  --dry-run       只打印将要执行的变更，不写文件
  --keep-claude   不修改 ~/.claude/settings.json，不删除 Claude hook 软链
  --keep-codex    不修改 ~/.codex/hooks.json，不删除 Codex hook 软链
...
remove_symlink_if_present() {
    ...
    if [ -L "$link" ]; then
        if [ "$DRY_RUN" -eq 1 ]; then
            log "将删除 $label 软链: $link -> $(readlink "$link")"
        else
            log "删除 $label 软链: $link -> $(readlink "$link")"
        fi
        run_cmd rm "$link"
    elif [ -e "$link" ]; then
        log "跳过 $label：$link 存在但不是软链，未删除"
    else
        log "$label 不存在: $link"
    fi
}
...
main() {
    ...
    if [ "$REMOVE_CLAUDE" -eq 1 ]; then
        remove_symlink_if_present "$CLAUDE_HOOK_LINK" "Claude hook"
    else
        log "跳过 Claude hook 软链"
    fi
    if [ "$REMOVE_CODEX" -eq 1 ]; then
        remove_symlink_if_present "$CODEX_HOOK_LINK" "Codex hook"
    else
        log "跳过 Codex hook 软链"
    fi
    ...
    log ""
    log "重新安装:"
    log "  bash $SCRIPT_DIR/install.sh"
}
