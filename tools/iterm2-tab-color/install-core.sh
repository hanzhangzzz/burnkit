...
会执行的变更:
  1. 复制 hook 脚本到 ~/.claude/hooks/tab_color_hook.sh
  2. 复制 hook 脚本到 ~/.codex/hooks/tab_color_hook.sh
...
for event_name in events:
    ...
    for group in groups:
        ...
        group["hooks"] = [
            h for h in before
            if "tab_color_hook.sh" not in h.get("command", "") or h.get("command", "") == hook_command
        ]
...
main() {
    ...
    install_hook_copy "$HOOK_SRC" "$CLAUDE_HOOK_LINK" "Claude hook"
    install_hook_copy "$HOOK_SRC" "$CODEX_HOOK_LINK" "Codex hook"
    ...
    log ""
    log "卸载命令:"
    log "  $REPO_ROOT/bin/burnkit uninstall tabs"
}
